...
import SeasonalPricing from './admin/SeasonalPricing'; // Importa il componente dei prezzi stagionali
...
const AdminPanelBasic = (): JSX.Element => {
  ...
  const updateSystemSettingValue = async (key: string, value: any) => {
    ...
    try {
      const result = await adminApiService.updateSystemSetting(key, value); // Corretto
      debugLog.log('✅ Impostazione aggiornata:', { key, value });
      return result;
    } catch (error) {
  ...
  const loadRealApiData = async () => {
    ...
    try { // Corretto
      debugLog.log('📄 Caricamento dati API...');
      ...
      debugLog.log('✅ Dati caricati con successo');
    } catch (error) {
  ...
  const toggleService = (service: any) => {
    ...
    // const servicesTotal = newServices.reduce((sum, s: any) => sum + parseFloat(s.price), 0); // Commented out as it's not used
  ...
  return (
    <AdminLayout
      ...
      role={role ?? undefined} // Use nullish coalescing for better type safety
    ...
    >
      <div className="admin-panel-basic admin-panel-fullheight">
        <main className="admin-main">
          ...
          {/* Prezzi Tab - Aggiunta sezione prezzi stagionali */}
          {activeTab === 'prezzi' && (
            <div className="admin-section admin-animate-fade-in">
              <h2>💰 Gestione Prezzi</h2>
              <p className="admin-section-description">
                Definisci regole di prezzo personalizzate per periodi specifici (es. alta stagione, festività).
              </p>
              <SeasonalPricing />
            </div>
